"""Enricher de destino via Groq — fallback para Gemini."""
...
class GroqDestinationEnricher(BaseDestinationEnricher):
    ...
    def _call_with_retry(self, prompt: str) -> dict[str, Any] | None:
        ...
        for attempt in (1, 2):
            try:
                ...
                logger.warning(
                    "Groq enrich tentativa %d falhou com LLMResponseError: %s",
                    attempt, exc,
                )
                ...
                logger.warning(
                    "Groq enrich falhou com %s: %s", type(exc).__name__, exc,
                )
                self._failures_so_far.append(
    ...
    def _build_result(self, payload: dict[str, Any]) -> EnrichmentResult:
        ...
        return EnrichmentResult(
            ...
            pois=pois,
            metadata={"model": settings.GROQ_MODEL, "source": "groq"},
