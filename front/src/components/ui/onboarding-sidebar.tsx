...
export default function OnboardingSidebar({ currentIndex }: OnboardingSidebarProps) {
  return (
    <aside className="flex w-full flex-col gap-6 overflow-hidden bg-[linear-gradient(160deg,#2e8cff_0%,#1553c6_45%,#0c2f73_100%)] p-6 text-white lg:m-4 lg:w-[22rem] lg:rounded-[28px] lg:p-8">
      <div className="hidden flex-col gap-3 lg:flex">
        <h1 className="text-3xl font-semibold leading-tight">Configure o seu DNA de Viajante</h1>
        <p className="text-sm leading-6 text-white/68">
          Suas preferencias alimentam a IA para recomendar destinos e montar roteiros
          feitos para voce.
        </p>
      </div>

      <nav className="flex flex-1 gap-2 overflow-x-auto lg:flex-col lg:gap-4" aria-label="Progresso do cadastro">
        {ONBOARDING_STEPS.map((step: OnboardingStep, i: number) => {
