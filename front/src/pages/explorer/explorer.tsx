...
function getDestinationSlug(itinerary: Itinerary) {
  if (typeof itinerary.destination === "object" && itinerary.destination?.slug) {
    return itinerary.destination.slug;
  }

  return itinerary.metadata?.destination_slug || itinerary.slug || String(itinerary.id);
}

function getDestinationCountry(itinerary: Itinerary) {
  if (typeof itinerary.destination === "object" && itinerary.destination?.country) {
...
function getTripImage(itinerary: Itinerary) {
  if (itinerary.metadata?.hero_image_url) return itinerary.metadata.hero_image_url;
  if (itinerary.metadata?.image_url) return itinerary.metadata.image_url;

  if (typeof itinerary.destination === "object" && itinerary.destination?.hero_image_url) {
    return itinerary.destination.hero_image_url;
  }

  if (typeof itinerary.destination === "object" && itinerary.destination?.image_url) {
...
function getTripRating(itinerary: Itinerary) {
  ...
  const destinationAverage =
    typeof itinerary.destination === "object" ? itinerary.destination?.average_rating : undefined;
...
function getTripHighlights(itinerary: Itinerary) {
  if (Array.isArray(itinerary.metadata?.highlights)) return itinerary.metadata.highlights;
...
function mapItineraryToTripRecommendation(itinerary: Itinerary): TripRecommendation {
  return {
    ...
    bestSeason:
      (typeof itinerary.destination === "object" && itinerary.destination?.best_season) ||
      itinerary.metadata?.best_season ||
      "Ano todo",
    travelStyle: itinerary.metadata?.travel_style || "Roteiro",
    generationStatus: (itinerary.generation_status as TripRecommendation["generationStatus"]) || "draft",
...
export default function Explorer() {
  ...
  const averageRating =
    trips.length > 0
      ? (
          trips.reduce((total, trip) => total + (Number.isFinite(trip.rating) ? trip.rating : 0), 0) /
          trips.length
        ).toFixed(1)
      : "0.0";
            ...
            <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
              <div className="max-w-3xl">
                ...
                <p className="mt-4 text-sm leading-6 text-neutral-600 md:text-base">
                  ...
                  or̤amento, dura̤̣o, estilo de viagem e pontos de interesse.
                </p>
              </div>
          ...
          <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
            <div>
              ...
              <p className="text-sm text-neutral-600">
                Sugest̵es carregadas da API para validar layout, filtros e navega̤̣o.
              </p>
            </div>
